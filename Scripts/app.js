...
var data = {
    ...
    aboutPage: {
        title: "About Me",
        button: "Resume",
        linkResume: "https://www.dropbox.com/s/at3djtjixix6lzk/AndresCorrea_Resume%20August-2022.pdf?dl=0",
        ...
        $.get("./Views/content/" + contentLink + ".html", function (html_data) {
            ...
            switch (contentLink) {
                ...
                case "experience":
                    ...
                    let jobbtnsContainer = document.querySelector("#experience .job-btns");
                    let jobDescription = document.querySelector("#experience .jobDescription p");
                    let logoUrl = document.querySelector("#experience figure img");
                    let companyName = document.querySelector("#experience .headings h5 ");
                    let jobDate = document.querySelector("#experience .headings span");
                    let jobTasks = document.querySelector("#experience .jobDescription ul");
                    let joinedTasks = "";
                    ...
                    let addInfo = (button) => {
                        ...
                        let jobSelected = data.experiencePage.find((job) => job.id === button.target.id);
                ...
                case "contact":
                    ...
                    $("#contact #name").attr("placeholder", `Type here your ${data.contactPage.name}`);
                    $("#contact .email").text(data.contactPage.email);
                    $("#contact .message").text(data.contactPage.message);
                    $("#contact button").html(`${data.contactPage.button}  <i class="fa-regular fa-paper-plane"></i>`);
                    let formValidation = {
                        name: false,
                        email: false,
                        message: false,
                    ...
                    let regExpressEmail = /^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,3})+$/;
                    $("#contact #email").on("change", (e) => {
                        const { target } = e;
                        // If the regular expression is false
                        if (!regExpressEmail.test(target.value)) {
